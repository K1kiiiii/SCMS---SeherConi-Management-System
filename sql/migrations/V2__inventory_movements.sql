...
-- Optional view for finished goods (products)
CREATE OR REPLACE VIEW finished_goods_available AS
SELECT product_id, SUM(CASE WHEN type = 'IN' THEN quantity ELSE -quantity END) AS boxes_available
FROM inventory_movements WHERE product_id IS NOT NULL
GROUP BY product_id;
