...
public class DashboardController {
    ...
    public void initialize() {
        ...
        loadTask.setOnSucceeded(evt -> {
            ...
            // start notification service for  magacioner
            try {
        ...
        //  refresh button and scroll panes don't show horizontal scroll
    ...
    private List<CardInfo> buildCardDataForRole() throws SQLException {
        ...
        switch (role) {
            case "ADMIN":
                ...
                break;
            case "MAGACIONER": // local DB role is 'magacioner' but treat as WAREHOUSE_STAFF
    ...
    // issued this month for a specific user
    ...
    // most frequently used material for a specific user
    ...
    private void onRequestIngredients(Task t) {
        try {
            ...
            //  user should not be able to create multiple requests for the same task
            try {
    ...
    private void onCompleteTask(Task t) {
        ...
        if (res.isPresent()) {
            Double produced = InputSanitizer.parseDoubleOrNull(res.get());
            if (produced == null) {
                showAlert(Alert.AlertType.WARNING, "Neispravan unos", "Količina mora biti broj.");
                return;
            }
            try {
                ...
                if (ok) {
                    try {
                        ...
                        if (created != null && created.getId() > 0 && verified) {
                            ...
                            showAlert(Alert.AlertType.INFORMATION, "Proizvod spremljen", "Proizvod je spremljen kao proizvod id=" + created.getId() + " za zadatak id=" + t.getId());
                        } else if (verified) {
