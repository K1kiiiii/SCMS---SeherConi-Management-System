...
public class MaterialManagementController {
    ...
    private void setupTableColumns() {
        ...
        colPrice.setCellValueFactory(new PropertyValueFactory<>("lastPurchasePrice"));
    ...
    private void handleRequestMaterial(ActionEvent event) {
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == reqButtonType) {
                try {
                    Double qty = InputSanitizer.parseDoubleOrNull(quantityField.getText());
                    if (qty == null) {
                    ...
                    String notes = InputSanitizer.sanitizeMultiline(notesArea.getText(), 2000);

                    assignmentService.requestMaterial(userId, selected.getId(), qtyVal, notes);
                    return true;
                } catch (ServiceException ex) {
    ...
    private Material showMaterialDialog(Material material) {
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == saveButtonType) {
                try {
                    Double quantity = InputSanitizer.parseDoubleOrNull(quantityField.getText());
                    if (quantity == null) {
                    ...
                    if (editMode) {
                        material.setName(InputSanitizer.sanitizeText(nameField.getText()));
                        material.setQuantity(quantity);
                        material.setUnit(InputSanitizer.sanitizeText(unitField.getText()));
                        material.setSupplier(InputSanitizer.sanitizeText(supplierField.getText()));
                        if (!purchasePriceField.getText().isBlank()) {
                            Double pp = InputSanitizer.parseDoubleOrNull(purchasePriceField.getText());
                            if (pp != null) material.setLastPurchasePrice(pp);
                        }
                        return material;
                    } else {
                        Material m = new Material();
                        m.setName(InputSanitizer.sanitizeText(nameField.getText()));
                        m.setQuantity(quantity);
                        m.setUnit(InputSanitizer.sanitizeText(unitField.getText()));
                        m.setSupplier(InputSanitizer.sanitizeText(supplierField.getText()));
                        if (!purchasePriceField.getText().isBlank()) {
                            Double pp = InputSanitizer.parseDoubleOrNull(purchasePriceField.getText());
                            if (pp != null) m.setLastPurchasePrice(pp);
                        }
