...
public class DatabaseConfig {
    ...
    public static void initDatabase() {
        ...
        try {
            ...
            try (Connection conn = DriverManager.getConnection(BASE_URL, USER, PASS);
                ...
                // inventory_movements table (tracks material/product IN/OUT movements)
                st.executeUpdate("""
                ...
                try { st.executeUpdate("CREATE INDEX IF NOT EXISTS idx_inv_mov_created_at ON inventory_movements(created_at)"); } catch (SQLException ignore) {}
                try { st.executeUpdate("CREATE INDEX IF NOT EXISTS idx_inv_mov_material ON inventory_movements(material_id)"); } catch (SQLException ignore) {}
                try { st.executeUpdate("CREATE INDEX IF NOT EXISTS idx_inv_mov_product ON inventory_movements(product_id)"); } catch (SQLException ignore) {}

                // Optional view for products available (aggregate IN/OUT by product)
                try {
                    st.executeUpdate("CREATE OR REPLACE VIEW finished_goods_available AS SELECT product_id, SUM(CASE WHEN type = 'IN' THEN quantity ELSE -quantity END) AS boxes_available FROM inventory_movements WHERE product_id IS NOT NULL GROUP BY product_id");
    ...
    // Ensure the inventory_movements table exists (safe to call multiple times)
    public static void ensureInventoryMovementsTableExists() {
        try (Connection conn = getConnection(); Statement st = conn.createStatement()) {
            ...
            System.out.println("ensureInventoryMovementsTableExists: ensured inventory_movements exists");
        } catch (SQLException ex) {
            System.err.println("Failed to ensure inventory_movements table: " + ex.getMessage());
        }
    }
